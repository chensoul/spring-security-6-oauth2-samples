...
/**
 * Helper class that performs steps of the
 * {@code urn:ietf:params:oauth:grant-type:device_code} flow using {@link MockMvc} for
 * testing.
 *
...
public class DeviceAuthorizationGrantFlow {
	...
	/**
	 * Perform the device authorization request and obtain the response containing a user
	...
	public Map<String, Object> authorize(RegisteredClient registeredClient) throws Exception {
		...
		MvcResult mvcResult = this.mockMvc
			.perform(MockMvcRequestBuilders.post("/oauth2/device_authorization").params(parameters).headers(basicAuth))
			.andExpect(MockMvcResultMatchers.status().isOk())
			.andExpect(MockMvcResultMatchers.header()
				.string(HttpHeaders.CONTENT_TYPE, Matchers.containsString(MediaType.APPLICATION_JSON_VALUE)))
	...
	public String submitCode(String userCode) throws Exception {
		...
		MvcResult mvcResult = this.mockMvc
			.perform(MockMvcRequestBuilders.get("/oauth2/device_verification")
				.queryParams(parameters)
				.with(SecurityMockMvcRequestPostProcessors.user(this.username).roles("USER")))
			.andExpect(MockMvcResultMatchers.status().isOk())
			.andExpect(MockMvcResultMatchers.header()
				.string(HttpHeaders.CONTENT_TYPE, Matchers.containsString(MediaType.TEXT_HTML_VALUE)))
			.andReturn();
	...
	public void submitConsent(RegisteredClient registeredClient, String state, String userCode) throws Exception {
		...
		MvcResult mvcResult = this.mockMvc
			.perform(MockMvcRequestBuilders.post("/oauth2/device_verification")
				.params(parameters)
				.with(SecurityMockMvcRequestPostProcessors.user(this.username).roles("USER")))
	...
	public Map<String, Object> getTokenResponse(RegisteredClient registeredClient, String deviceCode) throws Exception {
		...
		MvcResult mvcResult = this.mockMvc
			.perform(MockMvcRequestBuilders.post("/oauth2/token").params(parameters).headers(basicAuth))
			.andExpect(MockMvcResultMatchers.status().isOk())
			.andExpect(MockMvcResultMatchers.header()
				.string(HttpHeaders.CONTENT_TYPE, Matchers.containsString(MediaType.APPLICATION_JSON_VALUE)))
