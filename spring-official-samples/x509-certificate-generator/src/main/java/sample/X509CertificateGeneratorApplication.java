...
public class X509CertificateGeneratorApplication implements CommandLineRunner {
	...
	public void run(String... args) throws Exception {
		...
		// Generate the Root certificate (Trust Anchor or most-trusted CA) and keystore
		// file
		...
		X509Certificate rootCertificate = BouncyCastleUtils.createTrustAnchorCertificate(rootKeyPair,
				distinguishedName);
		writeCertificatePEMEncoded(rootCertificate,
				"./samples/x509-certificate-generator/generated/" + commonName + ".pem");
		writeKeystore(rootKeyPair, new Certificate[] { rootCertificate }, commonName, null,
				"./samples/x509-certificate-generator/generated/" + commonName + "-keystore.p12");
		TrustedCertificateHolder[] rootTrustedCertificate = {
		...
		X509Certificate caCertificate = BouncyCastleUtils.createCACertificate(rootCertificate, rootKeyPair.getPrivate(),
				caKeyPair.getPublic(), distinguishedName);
		writeCertificatePEMEncoded(caCertificate,
				"./samples/x509-certificate-generator/generated/" + commonName + ".pem");
		...
		X509Certificate demoClientCertificate = BouncyCastleUtils.createEndEntityCertificate(caCertificate,
		...
		writeKeystore(demoClientKeyPair, new Certificate[] { demoClientCertificate, caCertificate, rootCertificate },
				commonName, caTrustedCertificate, "./samples/demo-client/src/main/resources/keystore.p12");
		...
		X509Certificate selfSignedDemoClientCertificate = BouncyCastleUtils
			.createTrustAnchorCertificate(selfSignedDemoClientKeyPair, distinguishedName);
		...
		TrustedCertificateHolder[] trustedCertificates = { caTrustedCertificate[0],
		...
		X509Certificate messagesResourceCertificate = BouncyCastleUtils.createEndEntityCertificate(caCertificate,
				caKeyPair.getPrivate(), messagesResourceKeyPair.getPublic(), distinguishedName);
		messagesResourceCertificate.verify(caCertificate.getPublicKey(), BC_PROVIDER);
		writeKeystore(messagesResourceKeyPair,
				new Certificate[] { messagesResourceCertificate, caCertificate, rootCertificate }, commonName,
				trustedCertificates, "./samples/messages-resource/src/main/resources/keystore.p12");

		// Generate the certificate and keystore file for the demo-authorizationserver
		// sample
		commonName = "demo-authorizationserver-sample";
		distinguishedName = "CN=" + commonName + ", " + baseDistinguishedName;
		KeyPair demoAuthorizationServerKeyPair = BouncyCastleUtils.generateRSAKeyPair();
		X509Certificate demoAuthorizationServerCertificate = BouncyCastleUtils.createEndEntityCertificate(caCertificate,
				caKeyPair.getPrivate(), demoAuthorizationServerKeyPair.getPublic(), distinguishedName);
		demoAuthorizationServerCertificate.verify(caCertificate.getPublicKey(), BC_PROVIDER);
		writeKeystore(demoAuthorizationServerKeyPair,
				new Certificate[] { demoAuthorizationServerCertificate, caCertificate, rootCertificate }, commonName,
